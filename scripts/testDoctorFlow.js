...
// Requires the server to already be running (npm run dev) in another terminal.
...
async function main() {
  ...
  if (addPatient.status === 201 && addPatient.data.patient?.user_id) {
    logPass(`Patient created — ${addPatient.data.patient.name} (${addPatient.data.patient.user_id})`);
    console.log('   Simulated SMS:', addPatient.data.simulated_sms.message);
  } else {
    logFail('Add patient failed', addPatient.data);
    return printSummary();
  }
  const patientId = addPatient.data.patient.user_id;
  ...
  if (packages.status === 200 && Array.isArray(packages.data) && packages.data.length > 0) {
    ...
    logFail('Fetching therapy packages failed (did you run npm run db:seed:package?)', packages.data);
    return printSummary();
  }
